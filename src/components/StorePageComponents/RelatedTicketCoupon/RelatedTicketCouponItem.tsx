...
function RelatedTicketCouponItem({ coupon }: { coupon: CouponProps }) {
    ...
    return (
        <>
            <div className={"w-[280px] bg-transparent flex items-center justify-between gap-0 mx-0 " + (locale === 'ar' ? 'flex-row' : 'flex-row-reverse')}>
                ...
                <div className="check flex flex-col justify-between items-center">
                    <div className='w-full h-fit text-xs mb-2 font-semibold'>
                        <p className="line-clamp-2">{coupon?.slug?.replace(/-/g, ' ')}{" "}{coupon?.discount_value}</p>
                    </div>
                    {
                        coupon?.type === "coupon" ? (
                            <div className="w-11/12 h-12 overflow-hidden border-1 border-dashed rounded-md border-main-500 flex items-center justify-center">
                                ...
                                <div className={'w-2/5 h-full px-1 bg-yellow-300 hover:bg-main-500 hover:text-white flex items-center justify-center text-[10px] text-center cursor-pointer '
                            ...
                            <div className="w-11/12 h-12 overflow-hidden border-1 border-dashed rounded-md border-main-500 flex items-center justify-center">
                                <div className='w-full h-full px-1 bg-yellow-300 hover:bg-main-500 hover:text-white flex items-center justify-center text-[10px] text-center cursor-pointer'
                                    onClick={() => setSelectedCoupon(coupon)}
                                >
                                    {t("Get Coupon")}
                                </div>
