...
function TicketCoupon() {
  ...
  const [couponsCategoriesList, setCouponsCategoriesList] = React.useState<{ id: number; name: string; slug: string; }[]>([]);
  ...
  const fetchCoupons = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(process.env.NEXT_PUBLIC_API_URL + 'home/latest-coupons');
      if (res.status !== 200) return;

      const coupons = res.data.data;
      setCouponsData(coupons);

      const categoriesList = coupons
        .map((coupon: LatestCoupons) => coupon.category)
        .flat();

      const uniqueCategories = Array.from(
        new Set(categoriesList.map((cat: CategoryItem) => cat?.id))
      )
        .map((id) => categoriesList.find((cat: CategoryItem) => cat?.id === id))
        .filter((cat: CategoryItem | null): cat is CategoryItem => cat !== null);

      setCouponsCategoriesList(uniqueCategories);

      if (uniqueCategories.length > 0) {
        const firstCatId = uniqueCategories[0]?.id;
        setSelectedCat(firstCatId);
        const filtered = coupons.filter((item: LatestCoupons) =>
    ...
    <Fragment>
      {
        !smallScreens ? (
            ...
            <div className="bg-[#efefef] container flex flex-col items-center gap-5 shadow-lg p-5">
              <div className="w-full h-full flex items-start justify-start overflow-hidden">
                <div className="w-1/5 h-full flex flex-wrap items-start justify-center gap-5 bg-green-500/0 px-5">
                  <div
                    className="w-full h-90 flex flex-col gap-3 justify-start items-start overflow-y-scroll whitespace-nowrap cursor-grab select-none scrollbar-visible "
                    ref={ref as any}
                  >
                    {
                      isLoading ? (
                        ...
                        couponsCategoriesList.map((category) => (
                          <div
                            ...
                            className={
                              ...
                              (selectedCat === category?.id ? ' bg-main-500 text-white shadow-none' : ' bg-gray-50 text-gray-800 shadow-md')
                            }
                            onClick={() => filterCoupons(category)}
                            role='button'
                            areia-label={`Filter by ${category?.name}`}
                          >
                            {category?.id === 0 ? t(`${category?.name}`) : category?.name}
                          </div>
                        ))
                      )
                    }
                      ...
                      filteredCoupons.length > 0 ? (
                        filteredCoupons.map((coupon) => (
                          <div className='w-fit h-fit' key={coupon?.id}>
                            <div className={"w-[320px] flex items-center justify-between gap-0 " + (locale === 'ar' ? 'flex-row' : 'flex-row-reverse')}>
                                ...
                                </div>
                                {
                                  coupon?.type === "coupon" ? (
                                    <div className="w-11/12 h-10 bg-green-500/0 overflow-hidden border-1 border-dashed rounded-md border-main-500 flex items-center justify-center">
                                      ...
                                      <div role='button' aria-label='Copy Coupon' className={'w-2/5 h-full px-1 bg-yellow-300 hover:bg-main-500 hover:text-white flex items-center justify-center text-xs text-center cursor-pointer '
                                    ...
                                    <div role='button' aria-label='Copy Coupon' className="w-11/12 h-10 bg-green-500/0 overflow-hidden border-1 border-dashed rounded-md border-main-500 flex items-center justify-center">
          ...
          <div ref={inViewRef}>
            <div className="bg-[#efefef] container flex items-start p-5 pb-0">
              <h2 className="text-lg font-semibold text-neutral-900 sm:text-xl md:text-2xl">
                {t("BestCoupons")}
              </h2>
            </div>
                ...
                >
                  {
                    couponsCategoriesList.map((category) => (
                      <Chip
                        ...
                        className={
                          'text-sm ' +
                          (selectedCat === category?.id ? ' bg-main-500 text-white' : ' bg-gray-50 text-gray-800')
                        }
              ...
              <div className="w-full h-fit flex flex-wrap items-center justify-center gap-5 bg-green-250/0 p-0">
                {
                  !isCouponLoading ? (
                    filteredCoupons?.length > 0 ? (
                      filteredCoupons.map((coupon) => (
                        <div className='w-fit h-fit' key={coupon?.id}>
                          <div className={"ticket w-[320px] flex items-center justify-between gap-0 " + (locale === 'ar' ? 'flex-row' : 'flex-row-reverse')}>
                            <div className="stub overflow-hidden">
                              <div className='w-[110px] aspect-square bg-white'>
                                <Image
                                  src={coupon?.store_image}
