...
import {
  isDetailNotFound,
  normalizeDetailSlug,
  validateCategoryDetail,
  validateCountryDetail,
  validateDetailRedirect,
...
export function createDetailLoader<T>(
  ...
  const loader = unstable_cache(
    async (slug: string): Promise<ApiResult<T>> => {
      const endpoint =
        (kind === "country" ? "home/country/" : "categories/") +
      ...
      for (let attempt = 0; ; attempt++) {
        ...
        if (![200, 301, 404].includes(response.status))
          throw new Error(
            `Public ${kind} detail unavailable: ${response.status}`,
          );
        const value: unknown = await response.json();
        if (response.status === 404) {
          if (!isDetailNotFound(value, kind))
            throw new Error("Unrecognized detail 404");
          return { kind: "not_found" };
        }
        if (response.status === 301)
          return {
            kind: "redirect",
            redirect_url: validateDetailRedirect(value),
          };
    ...
    ["public-detail-v1", base, kind],
    { revalidate, tags: [`public-${kind}-details`] },
  );
...
export const getCountryDetail = cache(
  createDetailLoader("country", validateCountryDetail),
);
export const getCategoryDetail = cache(
  createDetailLoader("category", validateCategoryDetail),
);
