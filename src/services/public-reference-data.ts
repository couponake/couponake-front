...
export function createPublicListLoader<T>(
  endpoint: string,
  tag: string,
  ...
  return unstable_cache(
    async () => {
      ...
      if (!response.ok)
        throw new Error(
          `Public reference data unavailable: ${response.status}`,
        );
    ...
    ["public-reference-data-v1", base, endpoint],
    { revalidate, tags: [tag] },
  );
}

export const getCategories = cache(
  createPublicListLoader(
...
export const getCountryOptions = cache(
  createPublicListLoader(
    // Laravel pagination treats -1 as a negative page size and drops a country.
    "home/countries?per_page=300",
    "public-country-options",
    validateCountryOptions,
    300,
  ),
);
