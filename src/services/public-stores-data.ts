  ...
  const load = unstable_cache(
    async (page: number) => {
      const response = await fetch(
        `${base}stores/all-stores?page=${page}&per_page=50`,
        {
          cache: "no-store",
          redirect: "manual",
          credentials: "omit",
      ...
      if (response.status !== 200)
        throw new Error(`Public stores unavailable: ${response.status}`);
      return {
        ...validateStoresPage(await response.json(), page),
        fetchedAt: Date.now(),
      };
    },
    ["public-stores-pages-v1", base],
    { revalidate, tags: ["public-stores-pages"] },
  );
  return (page: number) => {
    if (parseStorePage(String(page)) !== page)
